...
class Tensor(OpMixin):
  ...
  def cummax(self, axis:int=0) -> tuple[Tensor, Tensor]:
    ...
    ```
    """
    if self.ndim == 0: return self._cumalu(axis, Ops.MAX), Tensor.zeros(self.shape, dtype=dtypes.int32, device=self.device)
    values, n = self._cumalu(axis, Ops.MAX), int(self.shape[axis])
